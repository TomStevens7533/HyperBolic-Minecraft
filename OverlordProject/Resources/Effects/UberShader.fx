...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	if (gUseSpecularBlinn) {
		...
		float specLighting = saturate(dot(h, normal));

		if (gUseTextureSpecularIntensity) {
			specLighting = pow(specLighting, gShininess);
		}
		return h * specLighting;
	}
	return float3(0, 0, 0);
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	if (gUseSpecularPhong == true) {
		...
		float phongValue = gShininess;

		
		if (gUseTextureSpecularIntensity) {
			phoneValue *= gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r ;
		}
		else {
			specularColor = gColorSpecular;
		}
...
{
	float3 sampledNormal = float3(0, 0, 0);
	float3 binormal = cross(tangent, normal);
	float3x3 localAxis = float3x3(tangent, binormal, normal);

	if (gUseTextureNormal == true)
	{
		sampledNormal = gTextureNormal.Sample(gTextureSampler, texCoord).rgb;
		sampledNormal.x = (2 * sampledNormal.x) - 1;
		sampledNormal.y = (2 * sampledNormal.y) - 1;
		sampledNormal.z = (2 * sampledNormal.z) - 1;
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	if (gUseFresnelFalloff) {
		...
		dotProduct = saturate(abs(dotProduct));
		float fresnel = 1 - dotProduct; //Get the inverse
		fresnel = pow(fresnel, gFresnelPower);
		fresnel = mul(fresnel, gFresnelMultiplier); //Multiplying with the FresnelMultiplier will adjust the brightness of the falloff.

		float fresnelMask = dot(float3(0, -1, 0), normal); //fresnelMask
		fresnelMask = 1 - saturate(fresnelMask); //Clamnp value and invert it
		if (gFresnelHardness > 0) {
			fresnelMask = pow(fresnelMask, gFresnelHardness);
		}

		fresnel = fresnel * fresnelMask;
		if (!gUseEnvironmentMapping) {
			float3 newColor = gColorFresnel * fresnel; //multiple color of fresnel with calculation;
			return  newColor;
		}
		else {
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 enviromentColor = float3(0, 0, 0);

	if (gUseEnvironmentMapping) {
		float3 relfectVector = reflect(viewDirection, normal);
		enviromentColor = gCubeEnvironment.Sample(gTextureSampler, relfectVector);
...
float4 MainPS(VS_Output input) : SV_TARGET{
	...
	//NORMAL
	float3 newNormal = CalculateNormal(normalize(input.Tangent), normalize(input.Normal), input.TexCoord);
	...
	//ENVIRONMENT MAPPING
	float3 environmentColor = CalculateEnvironment(viewDirection, input.Normal);

	//FRESNEL FALLOFF
	environmentColor = CalculateFresnelFalloff(input.Normal, viewDirection, environmentColor);
