...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	//1. Figure out the correct ShadowGeneratorType (either Static, or Skinned) with information from the incoming MeshFilter
		if (pMeshFilter->HasAnimations()) {
			...
			MaterialTechniqueContext pskinMat = m_GeneratorTechniqueContexts[(int)ShadowGeneratorType::Skinned];
			pMeshFilter->BuildVertexBuffer(sceneContext, pskinMat.inputLayoutID, pskinMat.inputLayoutSize, pskinMat.pInputLayoutDescriptions);
			return;
	}
	MaterialTechniqueContext pStatMat = m_GeneratorTechniqueContexts[(int)ShadowGeneratorType::Static];
	pMeshFilter->BuildVertexBuffer(sceneContext, pStatMat.inputLayoutID, pStatMat.inputLayoutSize, pStatMat.pInputLayoutDescriptions);
}
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//m_LightVP = sceneContext.pLights->GetDirectionalLight().direction * m_pShadowRenderTarget->
	...
	XMMATRIX  proj = DirectX::XMMatrixOrthographicLH(sceneContext.aspectRatio * 250.f, 250.f, 0.1f, 500.f);
	...
	FXMVECTOR lightDir = XMLoadFloat4(&sceneContext.pLights->GetDirectionalLight().direction);


	FXMVECTOR eyePosition = lightPos;
	FXMVECTOR focusPosition = lightPos + lightDir;
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	//1. Figure out the correct ShadowGeneratorType (Static or Skinned)
		if (pMeshFilter->HasAnimations()) {
			...
			pMatContext = m_GeneratorTechniqueContexts[(int)ShadowGeneratorType::Skinned];
	...
	//		- Perform Draw Call (same as usual, iterate Technique Passes, Apply, Draw - See ModelComponent::Draw for reference)
		...
		for (const auto& subMesh : pMeshFilter->GetMeshes())
		{
			...
			//Set Vertex Buffer
			const UINT offset = 0;
			const auto& vertexBufferData = pMeshFilter->GetVertexBufferData(pMatContext.inputLayoutID, subMesh.id);
