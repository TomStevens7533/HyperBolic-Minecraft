...
void ControllerComponent::Initialize(const SceneContext&)
{
	if(!m_IsInitialized)
	{
		auto pos = GetTransform()->GetPosition();
		PxExtendedVec3 descPos;
		descPos.x = pos.x;
		descPos.y = pos.y;
		descPos.z = pos.z;


		m_ControllerDesc.position = descPos;
		m_ControllerDesc.userData = this;

		auto proxy = GetGameObject()->GetScene()->GetPhysxProxy();
		//Get controller manager
		auto controllerManager = proxy->GetControllerManager();
		//create controller
