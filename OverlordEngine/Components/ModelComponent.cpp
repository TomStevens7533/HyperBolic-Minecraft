...
void ModelComponent::ShadowMapDraw(const SceneContext& sceneContext)
{
	...
	if (m_pMeshFilter->HasAnimations()) {
		ShadowMapRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, m_pGameObject->GetTransform()->GetWorld(), m_pAnimator->GetBoneTransforms());

		return;
	}
	ShadowMapRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, m_pGameObject->GetTransform()->GetWorld());
