...
{
	m_pChunkMeshFilter = ContentManager::Load<MeshFilter>(m_AssetFile);
	m_pChunkMeshFilter->BuildIndexBuffer(sceneContext);

	if (m_pChunkMeshFilter->m_HasAnimations)
		m_pAnimator = new ModelAnimator(m_pChunkMeshFilter);

	if (m_MaterialChanged && m_pMaterial)
	{
		m_pChunkMeshFilter->BuildVertexBuffer(sceneContext, m_pMaterial);
...
void ModelComponent::SetMaterial(BaseMaterial* pMaterial)
{
	...
	if(m_IsInitialized && GetScene())
	{
		...
		m_pChunkMeshFilter->BuildVertexBuffer(GetScene()->GetSceneContext(), m_pMaterial);
