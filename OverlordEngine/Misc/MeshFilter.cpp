...
MeshFilter::~MeshFilter()
{
	...
	for(VertexBufferData& data : m_VertexBuffer)
	{
		data.Destroy();
	}
...
{
	for (UINT i = 0; i<m_VertexBuffer.size(); ++i)
	{
		if (m_VertexBuffer[i].InputLayoutID == inputLayoutId)
			return i;
	}
...
const VertexBufferData& MeshFilter::GetVertexBufferData(const SceneContext& sceneContext, BaseMaterial* pMaterial)
{
	...
	if (possibleBuffer < 0)
	{
		...
		return m_VertexBuffer.back();
	}

	return m_VertexBuffer[possibleBuffer];
}
